...
<header class="bg-gradient-to-r from-indigo-600 to-purple-700 shadow-lg">
    <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-3xl font-bold text-white">📝 Edit Timetable</h1>
                <p class="text-indigo-100 mt-1">Update timetable: {{ $timetable->name }}</p>
            </div>
            <div class="flex space-x-3">
                <a href="{{ route('institution-admin.timetables.show', $timetable) }}" class="bg-white text-indigo-600 hover:bg-indigo-50 font-semibold py-2 px-4 rounded-lg shadow transition">
                    👁️ View Timetable
                </a>
                <a href="{{ route('institution-admin.timetables.index') }}" class="bg-indigo-500 text-white hover:bg-indigo-400 font-semibold py-2 px-4 rounded-lg shadow transition">
                    ← Back to Timetables
                </a>
            </div>
        </div>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white shadow-lg rounded-xl overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200">
                ...
                <p class="text-sm text-gray-600 mt-1">Update the basic details for this institution-wide timetable.</p>
            </div>
            
            <form action="{{ route('institution-admin.timetables.update', $timetable) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    @enderror
                    <p class="mt-1 text-sm text-gray-500">This is an institution-wide timetable covering all courses and rooms.</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="semester" 
                            ...
                            <option value="Semester 1" {{ old('semester', $timetable->semester) == 'Semester 1' ? 'selected' : '' }}>Semester 1</option>
                            <option value="Semester 2" {{ old('semester', $timetable->semester) == 'Semester 2' ? 'selected' : '' }}>Semester 2</option>
                            <option value="Summer Session" {{ old('semester', $timetable->semester) == 'Summer Session' ? 'selected' : '' }}>Summer Session</option>
                        </select>
                        @error('semester')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <input type="date" 
                           name="week_start" 
                           id="week_start" 
                           value="{{ old('week_start', $timetable->week_start?->format('Y-m-d')) }}"
                ...
                <div class="flex justify-end space-x-3 pt-6 border-t border-gray-200">
                    <a href="{{ route('institution-admin.timetables.show', $timetable) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition duration-150 ease-in-out">
                        Cancel
                    </a>
        ...
        <div class="mt-8 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6">
            <h4 class="text-lg font-semibold text-blue-900 mb-4">📊 Current Timetable Status</h4>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                <div class="bg-white p-4 rounded-lg shadow-sm">
                    <div class="text-2xl mb-2">📅</div>
                    <strong class="text-blue-900">Status</strong>
                    <p class="text-blue-700 mt-1">
                        <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full {{ $timetable->status === 'published' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                ...
                <div class="bg-white p-4 rounded-lg shadow-sm">
                    <div class="text-2xl mb-2">📝</div>
                    <strong class="text-blue-900">Entries</strong>
                    <p class="text-blue-700 mt-1">{{ $timetable->entries()->count() }} scheduled sessions</p>
                </div>
                <div class="bg-white p-4 rounded-lg shadow-sm">
                    <div class="text-2xl mb-2">📅</div>
                    <strong class="text-blue-900">Last Updated</strong>
                    <p class="text-blue-700 mt-1">{{ $timetable->updated_at?->format('M j, Y') ?? 'Never' }}</p>
                </div>
            </div>
        </div>
    </div>
